...
useEffect(() => {
    fetchAll();
    // Si viene de /appointments/new?pet=X abrimos el modal
    const petId = searchParams.get("pet");
    if (petId) {
    setForm({ ...EMPTY_FORM, pet: petId });
    setShowModal(true);
    }
}, []);
...
return (
        ...
        <div className="appts-list">
            ...
            return (
                ...
                <div className="appt-date-box">
                    ...
                    {new Date(appt.requested_date).getDate()}
                    </span>
                    <span className="appt-month">
                    {new Date(appt.requested_date).toLocaleString("es-AR", { month: "short" })}
                    </span>
                ...
                <div className="appt-info">
                    ...
                    {appt.vet_notes && (
                    <p className="appt-notes">📋 {appt.vet_notes}</p>
                    )}
                </div>
                ...
                <div className="appt-actions">
                    ...
                    <>
                        ...
                        <button className="btn-icon danger" onClick={() => setCancelConfirm(appt.id)} title="Cancelar">✕</button>
                    </>
                    )}
                </div>
    ...
    <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        background: rgba(255,107,107,0.12); border-radius: 12px;
        padding: 10px 14px; min-width: 56px; flex-shrink: 0;
        }
        ...
        .appt-notes { font-size: 0.82rem; color: rgba(255,255,255,0.35); font-style: italic; }

        .appt-actions { display: flex; flex-direction: column; gap: 6px; flex-shrink: 0; }
        .btn-icon {
        background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.10);
        border-radius: 8px; padding: 7px 9px; cursor: pointer; font-size: 0.9rem;
        ...
        .cancel-overlay p { color: #fff; font-size: 0.95rem; font-weight: 700; }
        .cancel-btns { display: flex; gap: 10px; }
        .btn-ghost-sm {
        background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.12);
        color: rgba(255,255,255,0.6); border-radius: 8px; padding: 7px 16px;
        font-family: 'Nunito', sans-serif; font-weight: 700; cursor: pointer; font-size: 0.88rem;
        }
        .btn-danger-sm {
        background: linear-gradient(135deg, #ff6b6b, #ff4a4a);
        border: none; color: #fff; border-radius: 8px; padding: 7px 16px;
        font-family: 'Nunito', sans-serif; font-weight: 700; cursor: pointer; font-size: 0.88rem;
        }

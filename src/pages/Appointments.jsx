...
const EMPTY_FORM = { pet: "", clinic: "", requested_date: "", reason: "" };
...
export default function Appointments() {
    ...
    const openNew = () => { setEditingAppt(null); setForm(EMPTY_FORM); setError(""); setShowModal(true); };
    const openEdit = (appt) => {
        setEditingAppt(appt);
        setForm({
    ...
    const handleSubmit = async (e) => {
        ...
        if (!form.pet || !form.requested_date || !form.reason || !form.clinic) {
            setError("Mascota, clínica, fecha y motivo son obligatorios."); return;
        }
        setSaving(true); setError("");
        try {
            if (editingAppt) await updateAppointment(editingAppt.id, form);

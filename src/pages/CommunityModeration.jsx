...
const filters = [
  ...
  { value: 'actioned', label: 'Resueltos' },
  ...
  return (
    <main className="moderation-page">
      <div className="moderation-shell">
        ...
        <div className="community-toolbar community-card">{filters.map((item) => <button key={item.value} className={`filter-pill ${status === item.value ? 'active' : ''}`} onClick={() => setStatus(item.value)}>{item.label}</button>)}</div>
